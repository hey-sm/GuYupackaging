...
interface IProps extends React.HtmlHTMLAttributes<HTMLDivElement> {
  ...
  status?: string;
  listCheck?: React.ReactNode;
  block?: JSX.Element;
}

const Layout4: React.FC<IProps> = ({
  tableElement,
  headerTitle,
  tree,
  resize,
  ...
  // const [status, setStatus] = useState<string | undefined>(status1);
  return (
    <Container className="page-container">
      ...
      <Card size="small" className="right">
        <Wrapper {...props} className="page-container">
          ...
          {status === 'block' && (
            <div className="table-content block">{block}</div>
          )}
          {children}
        </Wrapper>
      </Card>
